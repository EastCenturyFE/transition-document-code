<template>
  <div id="draw">
    ...
    <div class="canvas-container">
      <canvas ref="canvas" @mousedown="mouseDonwHandler" @mousemove="mouseMoveHandler" @mouseup="mouseUpHandler" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initDrawAreaWH () {
      const { actionArea: [wp, hp], canvasWH: [w, h] } = this
    ...
    checkClickResult (click) {
      ...
      const itemIndex = bezierCurve.findIndex((item, i) => (pointIndex = checkIsClickItemPoint(item, i, click)) !== false)
    ...
    checkIsClickItemPoint (item, index, click) {
      ...
      const pointIndex = item.findIndex((point, i) => checkIsClickPoint(point, index, i, click) !== false)
    ...
    calcLineData () {
      ...
      const points = bezierCurve.map(item =>
        item.map(p => p ? changeAxis(p) : ''))
    ...
    changeAxis ([x, y]) {
      const { drawAreaOriginPos: [ox, oy], drawAreaWH: [w, h] } = this
