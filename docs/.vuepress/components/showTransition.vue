...
export default {
  ...
  methods: {
    ...
    initRect () {
      const { render, rectPercent: [ pw, ph ], data } = this
    ...
    initBezierCurve () {
      ...
      const bezierCurve = axisChangedPoints.reduce((curve, [endPoint, ctlPoint2], i) => {
        if (i === 0) return curve
    ...
    axisChange ([x, y]) {
      const { rectStartPoint: [ox, oy], rectWH: [w, h] } = this

      return [x * w + ox, y * h + oy]
    },
    initPoint () {
      const { render, rectWH: [rw, rh], data, pointColor, animationFrame } = this
